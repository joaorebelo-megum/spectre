...
DataType BinaryWithGravitationalWavesVariables<DataType>::integrate_term(
  ...
  for (size_t k = 0; k < x.get(0).size(); ++k) {
    auto integrand = [this, i, j, k, &this_interpolation_position,
                      &this_interpolation_momentum,
                      &this_mass](const double t) {
      ...
      const double term1 =
          t / (this_distance_at_t * this_distance_at_t * this_distance_at_t) *
          ((-5. * dot(u1, u1) +
...
Scalar<DataType> BinaryWithGravitationalWavesVariables<DataType>::get_t_lapse(
  ...
  */
  // Boosted lapse from each boosted solution
...
BinaryWithGravitationalWavesVariables<DataType>::get_t_shift(DataType t) const {
  ...
  */
  // Boosted shifts from each boosted solution
  const auto spacetime_metric_left = get_t_boosted_spacetime_metric_left(t);
  const auto conformal_metric_left = gr::spatial_metric(spacetime_metric_left);
  const auto inv_conformal_metric_left =
      determinant_and_inverse(conformal_metric_left).second;
...
tnsr::aa<DataType, 3> BinaryWithGravitationalWavesVariables<
  ...
  */
  ...
  get(gamma_rr) =
      1. + 2. * mass_left_in / (mass_left_in + get(distance_left_t)) +
      square(2. * mass_left_in / (mass_left_in + get(distance_left_t))) +
      cube(2. * mass_left_in / (mass_left_in + get(distance_left_t)));
  ...
  for (size_t i = 0; i < 3; ++i) {
    for (size_t j = 0; j < 3; ++j) {
      spatial_metric.get(i, j) =
          (get(gamma_rr) - square(1. + mass_left_in / get(distance_left_t))) *
          normal_left_t.get(i) * normal_left_t.get(j);
    }
    spatial_metric.get(i, i) +=
        square(1. + mass_left_in / get(distance_left_t));
  }
...
tnsr::aa<DataType, 3> BinaryWithGravitationalWavesVariables<
  ...
  */
  ...
  Scalar<DataType> gamma_rr{t.size()};
  get(gamma_rr) =
      1. + 2. * mass_right_in / (mass_right_in + get(distance_right_t)) +
      square(2. * mass_right_in / (mass_right_in + get(distance_right_t))) +
      cube(2. * mass_right_in / (mass_right_in + get(distance_right_t)));
  get(lapse) = 1. / sqrt(get(gamma_rr));
  ...
  for (size_t i = 0; i < 3; ++i) {
    for (size_t j = 0; j < 3; ++j) {
      spatial_metric.get(i, j) =
          (get(gamma_rr) - square(1. + mass_right_in / get(distance_right_t))) *
          normal_right_t.get(i) * normal_right_t.get(j);
    }
    spatial_metric.get(i, i) +=
        square(1. + mass_right_in / get(distance_right_t));
  }
...
void BinaryWithGravitationalWaves::observer_vector(
  ...
  for (size_t i = 0; i < 3; ++i) {
    past_dt_position_left_.at(i).push_back(-dxdt.at(i) * mass_right());
    past_dt_position_right_.at(i).push_back(dxdt.at(i) * mass_left());
  }
  for (size_t i = 3; i < 6; ++i) {
