...
void test_suite(const elliptic::BoundaryConditionType& boundary,
  ...
  {
    INFO("Properties");
    if (boundary == elliptic::BoundaryConditionType::Dirichlet) {
    ...
    } else if (boundary == elliptic::BoundaryConditionType::Neumann) {
      CHECK(boundary_condition.boundary_condition_types() ==
            std::vector<elliptic::BoundaryConditionType>{
  ...
  }
  {
    ...
    if (boundary == elliptic::BoundaryConditionType::Dirichlet) {
      ...
      CHECK_ITERABLE_APPROX(get<0>(shift_excess),
                            get<0>(expected_shift_excess));
      CHECK_ITERABLE_APPROX(get<1>(shift_excess),
                            get<1>(expected_shift_excess));
      CHECK_ITERABLE_APPROX(get<2>(shift_excess),
                            get<2>(expected_shift_excess));
    } else if (boundary == elliptic::BoundaryConditionType::Neumann) {
      const auto expected_n_dot_conformal_factor_gradient =
          pypp::call<Scalar<DataVector>>(
              py_module, "n_dot_conformal_factor_gradient", x, face_normal);
      const auto expected_n_dot_lapse_times_conformal_factor_gradient =
          pypp::call<Scalar<DataVector>>(
              py_module, "n_dot_lapse_times_conformal_factor_gradient", x,
              face_normal);
...
SPECTRE_TEST_CASE("Unit.Xcts.BoundaryConditions.SuperposedBoostedBinary",
  ...
  test_suite<false>(elliptic::BoundaryConditionType::Dirichlet,
  ...
  test_suite<false>(elliptic::BoundaryConditionType::Neumann,
                    "SuperposedBoostedBinary:\n"
                    "  XCoords: [-5., 6.]\n"
                    "  Masses: [1.1, 0.43]\n"
                    "  MomentumLeft: [0.01, 0.01, 0.01]\n"
